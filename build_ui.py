...
读取 data/ 目录下的 YAML 数据和图片，生成自包含的 HTML 文件。
输出：ui/outfit.html（单文件，浏览器直接打开）
"""
...
.header .stats-bar{display:flex;gap:24px;margin-top:16px}
...
.tabs{display:flex;gap:4px;margin-bottom:24px;background:var(--card);padding:4px;border-radius:var(--radius);box-shadow:var(--shadow)}
.tab{padding:10px 20px;border:none;background:none;cursor:pointer;border-radius:8px;font-size:14px;font-weight:500;color:var(--dim);transition:.2s}
...
@media(max-width:600px){.header{padding:20px}.header h1{font-size:22px}.grid{grid-template-columns:repeat(2,1fr);gap:10px}.card-body{padding:10px}.card-name{font-size:13px}}
</style>
</head>
<body>
<div class="header">
...
function renderStats(){
  ...
  document.getElementById('profile-summary').textContent=
    (p.body?.height||'?')+'cm / '+(p.body?.weight||'?')+'kg · '+(p.location||'')+' · '+(p.lifestyle?.occupation||'');
...
function renderGrid(filter){
  ...
  if(filter&&filter!=='all'){
    ...
    else if(k==='style')items=items.filter(i=>(i.style||[]).includes(v));
  }
  if(!items.length){
    document.getElementById('wardrobe-grid').innerHTML='<div class="empty"><div class="empty-icon">👔</div>衣橱为空</div>';
    return;
  }
  document.getElementById('wardrobe-grid').innerHTML=items.map(item=>{
    const img=D.images[item.id];
    const ch=CM[item.colors?.primary]||'#ccc';
    ...
    return '<div class="card" onclick="showDetail(\''+item.id+'\')">'+imgH+
      '<div class="card-body"><div class="card-type">'+item.type+' · '+item.sub_type+'</div>'+
      '<div class="card-name">'+(item.colors?.primary||'')+' '+item.sub_type+'</div>'+
      '<div class="card-color"><span class="card-color-dot" style="background:'+ch+'"></span>'+
      (item.colors?.primary||'')+(item.colors?.secondary?' / '+item.colors.secondary:'')+'</div>'+
      '<div class="card-tags">'+tags+'</div>'+
      '<div class="card-meta"><span>🌡️ '+(item.temperature_range||'?')+'</span><span>穿 '+(item.wear_count||0)+' 次</span></div>'+
      '</div></div>';
  }).join('');
}
...
function showDetail(id){
  ...
  const imgH=img?'<img class="modal-img" src="'+img+'" alt="'+item.sub_type+'">':
    '<div class="modal-img" style="display:flex;align-items:center;justify-content:center;font-size:72px;color:#ccc;background:#f0efe9">👕</div>';
  ...
  document.getElementById('modal-content').innerHTML=imgH+
    '<div class="modal-body"><h2>'+(item.colors?.primary||'')+' '+item.sub_type+'</h2>'+
    '<div class="meta">'+item.type+' · '+(item.material||'?')+' · '+(item.fit||'?')+'</div>'+
    '<div class="section"><div class="section-title">风格</div><div class="card-tags">'+sty+'</div></div>'+
    '<div class="section"><div class="section-title">季节 · 场合</div><div class="card-tags">'+sea+' '+occ+'</div></div>'+
    '<div class="section"><div class="section-title">适穿温度</div><div>🌡️ '+(item.temperature_range||'?')+' ℃</div></div>'+
    '<div class="section"><div class="section-title">推荐搭配</div><div class="pair-list">'+(pair||'<span style="color:#999">暂无</span>')+'</div></div>'+
    (rst?'<div class="section"><div class="section-title">搭配禁忌</div>'+rst+'</div>':'')+
    '<div class="section"><div class="section-title">穿着记录</div><div>穿 '+(item.wear_count||0)+' 次 · '+(item.last_worn||'未穿过')+'</div></div>'+
    '</div>';
...
function renderProfile(){
  ...
  document.getElementById('tab-profile').innerHTML=
    '<div class="profile-card"><h2>👤 个人画像</h2><div class="profile-grid">'+
...
function renderRecommend(){
  const items=D.items;
  if(items.length<2){
    document.getElementById('tab-recommend').innerHTML='<div class="empty"><div class="empty-icon">✨</div>衣物太少，无法生成推荐</div>';
    return;
  }
  ...
  for(const t of tops)for(const b of bottoms){
    const c=[t,b];
    const a=accs.find(a=>(a.occasion||[]).some(o=>(t.occasion||[]).includes(o)||(b.occasion||[]).includes(o)));
    if(a)c.push(a);
    let sc=60;
    const safe=['纯白','白色','米白','米白色','黑色','炭黑','深浅灰','灰色','藏青','藏青色','卡其色'];
    if(safe.includes(t.colors?.primary))sc+=10;
    if(safe.includes(b.colors?.primary))sc+=10;
    const so=[...new Set(t.season||[])].filter(s=>(b.season||[]).includes(s));
    if(so.length)sc+=8;
    const st=[...new Set(t.style||[])].filter(s=>(b.style||[]).includes(s));
    if(st.length)sc+=7;
    if(((t.wear_count||0)+(b.wear_count||0))/2<3)sc+=5;
    combos.push({items:c,score:Math.min(sc,100)});
  }
  combos.sort((a,b)=>b.score-a.score);
  const top3=combos.slice(0,3);
  const gc=s=>s>=85?'#2d5a27':s>=70?'#5a8a27':s>=55?'#aa8a27':'#aa4427';
  document.getElementById('tab-recommend').innerHTML=
    '<div class="recommend-card"><h2>✨ 今日推荐穿搭</h2>'+
    '<p style="color:var(--dim);margin-bottom:16px;font-size:13px">基于你的风格偏好（'+(D.profile.style?.primary||[]).join('/')+'）自动搭配</p>'+
    top3.map((c,idx)=>'<div class="combo">'+
      '<div class="combo-score" style="color:'+gc(c.score)+'">'+c.score+'<div style="font-size:12px;font-weight:400">方案'+(idx+1)+'</div></div>'+
      '<div class="combo-items">'+c.items.map(item=>{
        const img=D.images[item.id];
        return img?'<img class="combo-thumb" src="'+img+'" title="'+item.sub_type+'">':'<div class="combo-thumb" style="display:flex;align-items:center;justify-content:center;font-size:20px">👕</div>';
      }).join('')+'</div>'+
      '<div class="combo-info"><div style="font-weight:600">'+c.items.map(i=>i.sub_type).join(' + ')+'</div>'+
      '<div style="font-size:12px;color:var(--dim)">'+c.items.map(i=>i.colors?.primary).join(' · ')+'</div></div></div>'
    ).join('')+'</div>';
}
...
function renderHistory(){
  const r=D.history||[];
  if(!r.length){
    document.getElementById('tab-history').innerHTML='<div class="empty"><div class="empty-icon">📅</div>暂无穿搭记录</div>';
    return;
  }
  document.getElementById('tab-history').innerHTML='<div class="profile-card"><h2>📅 穿搭历史</h2>'+
    r.map(rec=>'<div style="padding:12px;border-bottom:1px solid var(--border)">'+
      '<div style="font-weight:600">'+rec.date+'</div>'+
      '<div style="font-size:13px;color:var(--dim)">'+(rec.items||[]).join(', ')+(rec.occasion?' · '+rec.occasion:'')+'</div></div>'
    ).join('')+'</div>';
}

renderStats();renderFilters();renderGrid('all');renderProfile();renderRecommend();renderHistory();
