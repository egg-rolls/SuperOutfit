...
spof data export [-o output.zip]             # 导出数据
spof data import -f backup.zip [--merge]     # 导入数据
...
数据目录通过 `spof data dir` 命令配置，存储在 `~/.superoutfit/config.json`：

```bash
spof data dir "C:\Users\Lirui\AppData\Local\SuperOutfit\data"
```
