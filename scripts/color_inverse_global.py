...
给定部分颜色和目标分数，用全局优化推导补全的颜色。

用法：
  python color_inverse_global.py --known "#F5F0E8,#111111" --target 75 --missing 2
"""
...
def global_inverse(known_hex, target_score, missing_count, maxiter=1000):
    """
    全局优化反向推导
    ...
    result = differential_evolution(
        ...
        tol=1e-6,
        seed=42,
        workers=1,  # 单线程，避免序列化问题
    )

    # 提取结果
    unknown_rgb = result.x
...
def diverse_global_inverse(known_hex, target_score, missing_count, n_samples=3, maxiter=500):
    """
    多样化全局优化：生成多个不同的建议

    通过多次优化，使用不同的随机种子，得到多个不同的结果。
    """
    print(f"\n=== 多样化全局优化 ===")
    ...
    for i in range(n_samples):
        ...
        results = global_inverse(known_hex, target_score, missing_count, maxiter)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="色彩反向推导 — 全局优化版本")
    ...
    parser.add_argument("--maxiter", type=int, default=500, help="最大迭代次数")

    args = parser.parse_args()

    known_colors = [c.strip() for c in args.known.split(",")]
    diverse_global_inverse(known_colors, args.target, args.missing, args.samples, args.maxiter)
